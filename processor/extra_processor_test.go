...
func TestExtraProcessor_Act0(t *testing.T) {
	RegisterGlobalActionFunc("extra_send1", func(s *types.Session, key string, data interface{}, params []interface{}) (interface{}, error) {
		return pb.LogRecord{Url: params[0].(string), Method: params[1].(string)}, nil
	})
	var units = []*Unit{
		{
			Key: ".", Actions: []*Actor{
	...
	assert.Equal(t, ds[0]["extra_send1"][0].(pb.LogRecord).Url, "url1")
	assert.Equal(t, ds[0]["extra_send1"][0].(pb.LogRecord).Method, "method1")
	assert.Equal(t, ds[0]["extra_send1"][1].(pb.LogRecord).Url, "url2")
	assert.Equal(t, ds[0]["extra_send1"][1].(pb.LogRecord).Method, "method2")
}
...
func TestExtraProcessor_Act1(t *testing.T) {
	...
	assert.Equal(t, ds[0]["extra_send1"][0].(pb.LogRecord).Url, "url1")
	assert.Equal(t, ds[0]["extra_send1"][0].(pb.LogRecord).Method, "method1")
	assert.Equal(t, ds[0]["extra_send1"][1].(pb.LogRecord).Url, "url3")
	assert.Equal(t, ds[0]["extra_send1"][1].(pb.LogRecord).Method, "method3")
	assert.Equal(t, ds[1]["extra_send2"][0].(pb.LogRecord).Url, "url2")
	assert.Equal(t, ds[1]["extra_send2"][0].(pb.LogRecord).Method, "method2")
}
